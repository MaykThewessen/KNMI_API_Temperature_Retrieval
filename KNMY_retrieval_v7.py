...
fig = make_subplots(rows=2, cols=1, subplot_titles=(
    ...
    f"De Bilt: Max: {df['T_260'].max():.1f}°C, Min: {df['T_260'].min():.1f}°C, Avg: {df['T_260'].mean():.1f}°C<br><br>"
    f"Temperature Distribution"
))

# Temperature over time plot
fig.add_trace(go.Scatter(x=df.index, y=df["T_260"], mode='lines', name=f"KNMI Temperature at Station: {stations['name'][260]}"), row=1, col=1)
...
# Histogram plot
data_hist1 = df["T_260"]
fig.add_trace(go.Histogram(x=data_hist1, name=f"KNMI Temperature at Station: {stations['name'][260]}", autobinx=False, xbins=dict(start=min(data_hist1), end=max(data_hist1), size=2.0), histnorm=''), row=2, col=1)

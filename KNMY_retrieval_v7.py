...
start   = '20180101'
end = date.today()              # maximum requestable end date is 2d ago by KNMI API
end = end.strftime('%Y%m%d')

print(f"Retrieving KNMI data from {start} to {end}")
...
df = data_260
df['TD'] = df['TD'] / 10  # Convert temperature to Celsius TD = Dewpoint temperature
df['T'] = df['T'] / 10  # Convert temperature to Celsius T = Temperatuur (in 0.1 graden Celsius) op 1.50 m hoogte tijdens de waarneming ['C]

# T10N = Temperature at 10 cm above ground level ['C]
# T_380 = Temperature at station 380 (Maastricht) ['C]
...
end_retrieved = df['datetime'].max().strftime('%Y%m%d')
print(f"Data by KNMI received up to {end_retrieved}")
...
# Save data to excel
output_filename = f"KNMI_data_{stations_selected1}_{start}_{end_retrieved}.csv"

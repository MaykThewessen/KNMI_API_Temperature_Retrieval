...
#df = knmi.get_day_data_dataframe(stations, start, end, variables)

disclaimer, stations, variables, data = knmy.get_hourly_data(stations=stations_selected, start=start, end=end,inseason=inseason, variables=variables_selected, parse=parse)
...
plt.plot(df["T"]/10)    # , linewidth='0.2')
...
plt.legend(['Temp']) # legend entries as seperate strings in a list
plt.grid(True)
...
plt.legend(['Temperature']) # legend entries as seperate strings in a list
...
#%% Save data to excel
output_filename = f"KNMI_data_{stations['name'][260]}_{variables_selected}_{start}_{end}.xlsx"
